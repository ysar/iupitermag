...
pub fn assoc_legendre_poly(theta: f64, n_degree: usize) -> 
    (Array<f64, Ix2>, Array<f64, Ix2>) {
    
    let n_size = n_degree + 1;

    let mut p = Array::<f64, _>::zeros((n_size, n_size));
    let mut dp = Array::<f64, _>::zeros((n_size, n_size));
    let mut k = Array::<f64, _>::zeros((n_size, n_size));
    ...
    for i in 2..n_size {
        ...
        for j in 0..i {
            ...
            k[[i, j]] = ((i - 1).pow(2) - j.pow(2)) as f64 
                      / ((2 * i - 1) * (2 * i - 3)) as f64;
        ...
        dp[[i, i]] = 
            sintheta * dp[[i - 1, i - 1]] + costheta * p[[i - 1, i - 1]];
    }
    (p, dp)
}